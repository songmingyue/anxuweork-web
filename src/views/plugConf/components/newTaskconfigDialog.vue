...
const generateValidationRules = (configData: any[], pluginRules?: any[]) => {
  ...
  if (typeForm.value === 'step') {
    ...
    configData.forEach((config) => {
      if (config.fiedlInfos && Array.isArray(config.fiedlInfos)) {
        config.fiedlInfos.forEach((field) => {
          if (field.required) {
            rules[field.prop] = [
              {
                required: true,
  ...
  } else if (pluginRules && Array.isArray(pluginRules)) {
    // 对于其他形式，使用 pluginRule 数组中的验证规则
      ...
      if (rule.prop) {
        ...
        if (rule.min !== undefined || rule.max !== undefined) {
          ruleConfig.push({
            ...
            max: rule.max,
            message:
              rule.lengthMessage || `长度应在 ${rule.min || 0} 到 ${rule.max || 100} 个字符之间`,
            trigger: 'blur'
          })
        }
...
const processFormActiveSmall = computed(() => {
  ...
  if (newOptions[0] && newOptions[0].fiedlInfos && newOptions[0].fiedlInfos[0]) {
    ...
    // 添加调试日志
    console.log('原始选项数据:', originalOptions)
    console.log('过滤后选项数据:', filteredOptions)
    console.log('字段类型:', newOptions[0].fiedlInfos[0].type)
    console.log('字段属性:', newOptions[0].fiedlInfos[0].prop)
  }

  console.log('processFormActiveSmall result:', newOptions[0])
  return newOptions[0]
})
// 获取步骤状态
